class School < ApplicationRecord
    ...
    private 
    def self.q1max
      Question.joins(:musicin).where(:musicins => {formtype:1}).last.id
    end
    def self.q2max
       Question.joins(:musicin).where(:musicins => {formtype:2}).count * 2.00  
    end
    def self.q3max
        Question.joins(:musicin).where(:musicins => {formtype:3}).count * 2.00
    end
    def self.q4max
        Question.joins(:musicin).where(:musicins => {formtype:4}).count * 2.00
    end
end
